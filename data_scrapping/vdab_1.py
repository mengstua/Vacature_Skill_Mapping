...
@dataclass
class Config:
    SAVE_PATH: str = os.getenv("SAVE_PATH", "vdab_jobs_playwright_full_data2.csv")
    ...
    USE_REQUESTS_FOR_DETAILS: bool = os.getenv("USE_REQUESTS_FOR_DETAILS", "1") in ("1", "true", "True")
    # Use aiohttp for both listing and detail fetching (bypasses Playwright entirely).
    USE_AIOHTTP_FOR_FETCH: bool = os.getenv("USE_AIOHTTP_FOR_FETCH", "1") in ("1", "true", "True")
...
def extract_skills(text: str) -> Dict:
    ...
    skill_patterns = {
        "computer_skill": ["excel", "office", "erp", "software", "ict", "python", "java", "sql"],
        "ai_skill": ["machine learning", "ai", "kunstmatige intelligentie", "artificial intelligence"],
        "data_analysis_skill": ["data", "analyse", "statistiek", "analytics", "bi"],
        "communication_skill": ["communicatie", "communicatief", "presentatie", "overleg"],
        "leadership_skill": ["leiding", "manager", "coach", "teamleider", "leiderschap"],
        "project_management": ["project", "planning", "scrum", "agile", "waterfall"],
        "customer_service_skill": ["klantgericht", "klant", "service", "customer"],
        "sales_skill": ["verkoop", "sales", "commercieel", "accountmanager"],
        "technical_skill": ["technisch", "engineering", "installatie", "technologie"],
        "creative_skill": ["creatief", "ontwerp", "design", "grafisch"],
        "finance_skill": ["boekhouding", "finance", "budget", "accounting"],
        "hr_skill": ["hr", "rekrutering", "personeel", "human resources"],
        "administrative_skill": ["administratie", "secretariaat", "kantoor"],
        "dutch_language": ["nederlands", "dutch", "nederlandstalig"],
        "french_language": ["frans", "français", "franstalig", "french"],
        "english_language": ["engels", "english", "engelstalig"],
    }
    
    skills = {}
    for skill_key, keywords in skill_patterns.items():
        skills[skill_key] = "Yes" if any(keyword in text for keyword in keywords) else "No"
...
class VdabScraper:
    ...
    async def scrape_job_page(self, page, url: str, job_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Use a robust title extractor that prefers JSON-LD and avoids generic site titles
            title = self.extract_title(job_soup, url)
            company = self.extract_text(job_soup, ['.vdab-company', '.job-company', '[class*="company"]', 'strong'])
            city = self.extract_text(job_soup, ['.job-location', '.location', '[class*="location"]'])
            desc_text = job_soup.get_text(" ", strip=True)
            ...
            # JSON-LD fallback: if selectors failed, try structured data (JobPosting)
                    ...
                    for script in job_soup.select('script[type="application/ld+json"]'):
                        ...
                        for it in items:
                            ...
                            if typ == 'jobposting' or any(k in it for k in ('jobLocation', 'hiringOrganization', 'title')):
                                ...
                                # stop if we have gathered enough
            ...
            # Compute features from full description, then prefer salary from the 'anbod' section
            feats = extract_features(desc_text)
            try:
        ...
        try:
            ...
            if getattr(self.config, 'USE_SELECTOLAX', False) and SELECTOLAX_AVAILABLE:
                try:
                    ...
                    # full text
                    desc_text = doc.text(separator=' ').strip()
                except Exception:
                    ...
                    # Reject pages that are likely landing/search pages even if URL matches
                    if not self.is_likely_vacancy(job_soup, html, url, title):
                        logger.debug(f"Rejected non-vacancy HTML for {url} (title='{title}')")
                        return None
            else:
                job_soup = BeautifulSoup(html, 'lxml')
                title = self.extract_title(job_soup, url)
    ...
    async def get_domains(self, page) -> List[str]:
        ...
        try:
            ...
            for a in soup.find_all('a', href=True):
                ...
                if not href:
                    continue
                # normalize absolute-path links against site root to avoid joining into BASE_URL path
                if href.startswith('/'):
                ...
                if '/vindeenjob/jobs/' in full:
                    ...
                    # some pages use different patterns; attempt to extract a domain-like token
    ...
    async def get_domains_aiohttp(self, session: 'aiohttp.ClientSession') -> List[str]:
        ...
        try:
            ...
            for a in soup.select('a'):
                ...
                if not href:
                    continue
                # normalize relative links against the jobs base so we capture domain tokens
                full = urljoin(self.config.BASE_URL + '/', href) if not href.startswith('http') else href
                if '/vindeenjob/jobs/' in full or '/jobs/' in full:
                    # accept either '/vindeenjob/jobs/<domain>' or '/jobs/<domain>' style links
                    if '/vindeenjob/jobs/' in full:
                        domain = full.split('/vindeenjob/jobs/')[-1].strip().strip('/')
                    else:
                        domain = full.split('/jobs/')[-1].strip().strip('/')
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            ...
            async def fetch_listing(domain: str):
                ...
                while True:
                    ...
                    if not tiles:
                        # no tiles found -> end of pagination for this domain
                        break

                    links = []
                    for tile in tiles:
                        # Prefer explicit product-link anchors inside the tile
                        ...
                        # Normalize and accept vacancy-related pages (including listing/search pages)
                        parsed_path = urlparse(u).path or ''
                        # Accept only vacancy pages that include a numeric vacancy id (reduces landing/search pages)
                        # Example: /vindeenjob/vacatures/72071203/...
                        if not re.search(r"/vacatures/\d+", parsed_path):
                            # skip links that are not direct vacancy pages
                            continue
                        # avoid duplicates
                        if u in links:
                            continue
    ...
    async def scrape_domain(self, page, domain: str) -> List[Dict]:
        ...
        try:
            ...
            while offset < (self.config.MAX_PAGES * page_size):
                ...
                # parse rendered HTML and look for product tiles
                html = await page.content()
                soup = BeautifulSoup(html, 'lxml')
                tiles = soup.select('div.product-tile')
                ...
                if not tiles:
                    consecutive_empty += 1
                    logger.debug(f"No product tiles found for {domain} at {offset} (consecutive {consecutive_empty})")
                    # Stop after a few consecutive empty pages to avoid endless loops
                    if consecutive_empty >= getattr(self.config, 'EMPTY_PAGE_GUARD', 3):
                        logger.info(f"No product tiles found for {domain} at {offset} for {consecutive_empty} consecutive pages; stopping")
                        break
                    else:
                        offset += page_size
                        continue
                ...
                # log per-offset discovery
                logger.info(f"{domain} offset {offset}: discovered {len(new_links)} new job links; total_seen={len(seen)}")

                if not new_links:
                    # no more links on this offset -> end pagination
                    break
    ...
    def save_progress(self):
        ...
        if self.results:
            ...
            desired_columns = [
                ...
                'functieomschrijving','profiel','professionele_vaardigheden','anbod','full_description',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VDAB Playwright scraper")
